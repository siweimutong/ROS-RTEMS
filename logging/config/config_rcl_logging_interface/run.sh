...
source_file="$HOME/RTEMS/quick-start/app/workspace/logging/config/config_rcl_logging_interface/tmp/build/arm-rtems6-realview_pbx_a9_qemu/librcl_logging_interface.a"  # Source file (with full path)
...
if [ -f "${target_file}" ]; then
    rm -f "${target_file}"  # -f Force delete, avoid error when file is not found
