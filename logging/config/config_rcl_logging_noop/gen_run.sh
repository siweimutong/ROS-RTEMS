...
# python ../mk_to_wscript_static.py ../librcl_logging_noop.mk wscript
...
def build(bld):
    ...
    # rcl_logging_noop target (static library)
    # Compile and link static library
    bld.stlib(
        target="rcl_logging_noop",
        source=[
            "init.c",
            "../../../code/src/rcl_logging-2.3.1/rcl_logging_noop/src/rcl_logging_noop/rcl_logging_noop.cpp"
        ],
        includes=[
        ...
        linkflags="-static",
        lib=["rcl_logging_interface", "rcutils", "stdc++", "rtemscpu", "rtemsbsp", "rtemsdefaultconfig", "m", "gcc", "c"],
    )
' > wscript
