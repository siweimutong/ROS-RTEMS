...
public:
  ...
  provide_intra_process_message(ConstMessageSharedPtr message) override
  {
    ...
    #ifdef RTEMS
    struct timespec _ip_t0, _ip_t1;
    clock_gettime(CLOCK_MONOTONIC, &_ip_t0);
    #endif
    trigger_guard_condition();
    #ifdef RTEMS
    clock_gettime(CLOCK_MONOTONIC, &_ip_t1);
    g_rclcpp_gc_trigger_ns = (int64_t)((_ip_t1.tv_sec-_ip_t0.tv_sec)*1000000000LL+
                                        (_ip_t1.tv_nsec-_ip_t0.tv_nsec));
    _ip_t0 = _ip_t1;
    #endif
    this->invoke_on_new_message();
    #ifdef RTEMS
    clock_gettime(CLOCK_MONOTONIC, &_ip_t1);
    g_rclcpp_invoke_cb_ns = (int64_t)((_ip_t1.tv_sec-_ip_t0.tv_sec)*1000000000LL+
                                       (_ip_t1.tv_nsec-_ip_t0.tv_nsec));
    #endif
  }
...
protected:
  ...
  trigger_guard_condition() override
  {
    if (this->direct_notify_callback_) {
      /* Direct notification mode: skip guard condition.
       * The consumer task will be notified via direct_notify_callback_
       * (called in invoke_on_new_message()), bypassing rcl_wait entirely. */
    } else {
      /* Standard mode: trigger guard condition for executor detection via rcl_wait */
      this->gc_.trigger();
    }
  }
