...
#include <rtems.h>
#include <cstdio>
/* Exp2.1 instrumentation: per-call raw measurement of get_next_ready_executable */
volatile uint64_t g_get_next_ready_ns = 0;
volatile int g_get_next_ready_calls = 0;
volatile uint64_t g_get_next_ready_min_ns = (uint64_t)-1;
volatile uint64_t g_get_next_ready_max_ns = 0;
/* Exp2.1 instrumentation: measure wait_for_work overhead */
volatile uint64_t g_wait_for_work_ns = 0;
volatile int g_wait_for_work_calls = 0;
/* Exp2.1: per-call raw output control (set by test_main) */
volatile int g_exp21_raw_enabled = 0;
/* Exp2.1: end timestamp and duration of last successful get_next_ready_executable */
volatile uint64_t g_sel_end_ns = 0;
volatile uint64_t g_sel_dur_ns = 0;
/* Exp2.1: duration of last wait_for_work */
volatile uint64_t g_wait_dur_ns = 0;
/* Exp3-ST instrumentation: per-call timing globals (read by application) */
volatile int64_t g_rclcpp_gc_trigger_ns = 0;
volatile int64_t g_rclcpp_invoke_cb_ns = 0;
volatile int64_t g_rclcpp_wait_ns = 0;
volatile int64_t g_rclcpp_getnext_ns = 0;
volatile int64_t g_rclcpp_take_data_ns = 0;
volatile int64_t g_rclcpp_execute_ns = 0;
volatile int64_t g_rclcpp_execute_start_ns = 0;
/* Exp3-ST: flag set after intra-process publish, read in wait_for_work fast path */
...
#ifdef RTEMS
    struct timespec _ex_t0, _ex_t1;
    clock_gettime(CLOCK_MONOTONIC, &_ex_t0);
    g_rclcpp_execute_start_ns = (int64_t)(_ex_t0.tv_sec)*1000000000LL + (int64_t)(_ex_t0.tv_nsec);
#endif
    any_exec.waitable->execute(any_exec.data);
#ifdef RTEMS
    clock_gettime(CLOCK_MONOTONIC, &_ex_t1);
    g_rclcpp_execute_ns = (int64_t)((_ex_t1.tv_sec - _ex_t0.tv_sec)*1000000000LL +
                                     (_ex_t1.tv_nsec - _ex_t0.tv_nsec));
#endif
  }
...
//  TRACEPOINT(rclcpp_executor_wait_for_work, timeout.count());
#ifdef RTEMS
  struct timespec _wfw_t0, _wfw_t1;
  int64_t _rcl_wait_ns = 0;
  clock_gettime(CLOCK_MONOTONIC, &_wfw_t0);
#endif
  ...
  #ifdef RTEMS
  struct timespec _rw_t0, _rw_t1;
  rcl_ret_t status = RCL_RET_OK;
  if (g_intra_process_published) {
    g_intra_process_published = 0;
    for (size_t i = 0; i < wait_set_.size_of_guard_conditions; i++) {
      wait_set_.guard_conditions[i] = nullptr;
    }
    for (size_t i = 0; i < wait_set_.size_of_timers; i++) {
      wait_set_.timers[i] = nullptr;
    }
    for (size_t i = 0; i < wait_set_.size_of_clients; i++) {
      wait_set_.clients[i] = nullptr;
    }
    for (size_t i = 0; i < wait_set_.size_of_services; i++) {
      wait_set_.services[i] = nullptr;
    }
    for (size_t i = 0; i < wait_set_.size_of_events; i++) {
      wait_set_.events[i] = nullptr;
    }
  } else {
    clock_gettime(CLOCK_MONOTONIC, &_rw_t0);
    status = rcl_wait(&wait_set_, std::chrono::duration_cast<std::chrono::nanoseconds>(timeout).count());
    clock_gettime(CLOCK_MONOTONIC, &_rw_t1);
    _rcl_wait_ns = (int64_t)((_rw_t1.tv_sec - _rw_t0.tv_sec)*1000000000LL +
                              (_rw_t1.tv_nsec - _rw_t0.tv_nsec));
  }
  #else
  rcl_ret_t status =
...
#ifdef RTEMS
  clock_gettime(CLOCK_MONOTONIC, &_wfw_t1);
  int64_t _total_ns = (int64_t)((_wfw_t1.tv_sec - _wfw_t0.tv_sec)*1000000000LL +
                                 (_wfw_t1.tv_nsec - _wfw_t0.tv_nsec));
  g_rclcpp_wait_ns = _total_ns - _rcl_wait_ns;
  g_wait_for_work_ns += (uint64_t)(_total_ns - _rcl_wait_ns);
  g_wait_for_work_calls++;
  g_wait_dur_ns = (uint64_t)(_total_ns - _rcl_wait_ns);
#endif
}
...
{
#ifdef RTEMS
  struct timespec _gnr_t0, _gnr_t1;
  rtems_clock_get_uptime(&_gnr_t0);
#endif
...
#ifdef RTEMS
  if (success) {
    rtems_clock_get_uptime(&_gnr_t1);
    uint64_t t0 = (uint64_t)_gnr_t0.tv_sec * 1000000000ULL + (uint64_t)_gnr_t0.tv_nsec;
    uint64_t t1 = (uint64_t)_gnr_t1.tv_sec * 1000000000ULL + (uint64_t)_gnr_t1.tv_nsec;
    uint64_t dur = t1 - t0;
    g_rclcpp_getnext_ns = (int64_t)dur;
    g_get_next_ready_ns += dur;
    g_get_next_ready_calls++;
    if (dur < g_get_next_ready_min_ns) g_get_next_ready_min_ns = dur;
    if (dur > g_get_next_ready_max_ns) g_get_next_ready_max_ns = dur;
    g_sel_end_ns = t1;
    g_sel_dur_ns = dur;
    if (g_exp21_raw_enabled) {
      double dur_us = (double)dur / 1000.0;
      printf("[E2.1-CSV] %d,%.3f\n", g_get_next_ready_calls, dur_us);
    }
  }
#endif
