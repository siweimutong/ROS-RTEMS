...
def generate_rtems_wscript(config, output_file):
    ...
    if config['target']:
        ...
        if is_library:
            ...
            # Add dependency libraries (strip .so suffix)
...
def replace_src_with_source(file_path, strs, target):
    ...
    if modified:
        ...
        # print(f"already backed up the original file to: {backup_path}")
        
        # Write replaced content
        with open(file_path, 'w', encoding='utf-8') as f:
            f.writelines(new_lines)
        # print(f"Replacement done! Already replaced 'src' with 'source' in source/includes rows of file {file_path}")
    else:
