...
source_file="$HOME/RTEMS/quick-start/app/workspace/composition_interfaces/config/config_composition_interfaces__rosidl_typesupport_introspection_cpp/tmp/build/arm-rtems6-realview_pbx_a9_qemu/libcomposition_interfaces__rosidl_typesupport_introspection_cpp.a"  # Source file (with full path)
...
if [ -f "${target_file}" ]; then
    rm -f "${target_file}"  # -f forces deletion, avoiding an error if the file is not found
