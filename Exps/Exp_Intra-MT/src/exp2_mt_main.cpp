/**
 * @file exp2_mt_main.cpp
 *
 * Exp2-MT: Task Chain Latency — MultiThreadedExecutor
 *
 * CSV columns:
...
struct TimerNode : public rclcpp::Node
{
  ...
  void on_timer()
  {
    ...
    if (CHAIN_LENGTH == 1) {
      ...
      printf("[E2MT-CSV] %d,%u,%lld,0,0,0,0,0,0\n",
         ...
         rclcpp::NodeOptions().use_intra_process_comms(true))
  {
    ...
    sub_ = this->create_subscription<std_msgs::msg::Int32>(
      ...
      [](std_msgs::msg::Int32::UniquePtr msg) {
        ...
        printf("[E2MT-CSV] %d,%u,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld\n",
        ...
        g_round++;
        if (g_round >= TOTAL_ROUNDS + WARMUP_ROUNDS && !g_done.exchange(true)) {
          printf("[E2MT-SUMMARY] chain_len=%d "
...
int main(int argc, char * argv[])
{
  ...
  printf("=== Exp2-MT: Task Chain (n=%d) MultiThreadedExecutor ===\n", CHAIN_LENGTH);
  printf("Period: %dms, Rounds: %u\n", TIMER_PERIOD_MS, TOTAL_ROUNDS);
  printf("[E2MT-CSV] chain_len,round,e2e_ns,publish_ns,gc_trigger_ns,invoke_cb_ns,getnext_ns,take_data_ns,sched_ns,dispatch_ns\n");
