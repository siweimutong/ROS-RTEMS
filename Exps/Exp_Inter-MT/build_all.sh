#!/bin/bash
#
# build_all.sh — Compile once, then link all 4 chain lengths for Exp_Inter-MT
#
set -euo pipefail
...
# Verify object files (two source files per target)
IDX=1
for N in 2 3 4 5 10 15; do
    TEST_OBJ="$BUILD_DIR/src/test_main.cpp.${IDX}.o"
    MAIN_OBJ="$BUILD_DIR/src/exp3inter_mt_main.cpp.${IDX}.o"
...
echo "[2/2] Linking all executables..."
IDX=1
for N in 2 3 4 5 10 15; do
    TEST_OBJ="$BUILD_DIR/src/test_main.cpp.${IDX}.o"
    MAIN_OBJ="$BUILD_DIR/src/exp3inter_mt_main.cpp.${IDX}.o"
    OUTPUT="$BUILD_DIR/exp3inter_mt_n${N}.exe"
