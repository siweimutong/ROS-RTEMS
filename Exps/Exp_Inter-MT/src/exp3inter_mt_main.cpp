/**
 * @file exp3inter_mt_main.cpp
 *
 * Exp3inter-MT: Task Chain Latency — MultiThreadedExecutor
 *
 * CSV columns:
...
struct TimerNode : public rclcpp::Node
{
  ...
  void on_timer()
  {
    ...
    if (CHAIN_LENGTH == 1) {
      ...
      printf("[E3IMT-CSV] %d,%u,%lld,0,0,0,0,0,0\n",
         ...
         rclcpp::NodeOptions().use_intra_process_comms(false))
  {
    ...
    sub_ = this->create_subscription<std_msgs::msg::Int32>(
      ...
      [](std_msgs::msg::Int32::UniquePtr msg) {
        ...
        printf("[E3IMT-CSV] %d,%u,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld\n",
        ...
        g_round++;
        if (g_round >= TOTAL_ROUNDS && !g_done.exchange(true)) {
          printf("[E3IMT-SUMMARY] chain_len=%d "
...
int main(int argc, char * argv[])
{
  ...
  printf("=== Exp3inter-MT: Task Chain (n=%d) MultiThreadedExecutor ===\n", CHAIN_LENGTH);
  printf("Period: %dms, Rounds: %u\n", TIMER_PERIOD_MS, TOTAL_ROUNDS);
  printf("[E3IMT-CSV] chain_len,round,e2e_ns,publish_ns,gc_trigger_ns,invoke_cb_ns,getnext_ns,take_data_ns,sched_ns,dispatch_ns\n");
