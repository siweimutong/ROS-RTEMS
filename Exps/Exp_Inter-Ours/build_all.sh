#!/bin/bash
#
# build_all.sh — Compile once, then link all 6 chain lengths
# Uses RTExecutor from rclcpp-rtems with standard librclcpp.a
# rclcpp-rtems adds DDS notification to RTExecutor (dual intra-process + DDS paths)
#
set -euo pipefail
...
# rclcpp-16.0.8 librclcpp.a (rclcpp-rtems headers provide modified subscription_base.hpp)
...
echo "[1/3] Compiling all targets..."
...
for N in 2 3 4 5 10 15; do
    ...
    MAIN_OBJ="$BUILD_DIR/src/exp3inter_ours_main.cpp.${IDX}.o"
...
# Step 2: Link all 6 executables
LIBS=(
    -Wl,--start-group
    "$RCLCPP_LIB_DIR/librclcpp.a"
...
echo "[2/3] Linking all executables (rclcpp-rtems RTExecutor + standard librclcpp.a)..."
IDX=1
for N in 2 3 4 5 10 15; do
    MAIN_OBJ="$BUILD_DIR/src/exp3inter_ours_main.cpp.${IDX}.o"
    TEST_OBJ="$BUILD_DIR/src/test_main.cpp.${IDX}.o"
    IMPL_OBJ="$BUILD_DIR/src/ours_executor_impl.cpp.1.o"
    DRIVER_OBJ="$BUILD_DIR/driver/rtss_timer_driver.c.1.o"
    OUTPUT="$BUILD_DIR/exp3inter_ours_n${N}.exe"
...
echo "All 6 executables built successfully."
