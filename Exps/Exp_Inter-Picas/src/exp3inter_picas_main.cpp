/**
 * @file exp3inter_picas_main.cpp
 *
 * Exp3inter-Picas: Task Chain Latency — SingleThreadedExecutor + PiCAS
 *
 * Same chain topology as Exp3-ST, but uses rclcpp-picas with
 * priority-based callback scheduling (PiCAS) instead of the
 * standard FIFO dispatch.
 *
 * CSV columns:
 *   e2e         - first task start to last task end (callback tail)
 *   publish     - cumulative sum of all publish() call times across chain
 *   gc_trigger  - 0 (not instrumented in rclcpp-picas)
 *   invoke_cb   - 0 (not instrumented in rclcpp-picas)
 *   getnext     - 0 (not instrumented in rclcpp-picas)
 *   take_data   - 0 (not instrumented in rclcpp-picas)
 *   sched       - cumulative sum of all per-step publish-return-to-cb-entry delays
 *   dispatch    - 0 (not instrumented in rclcpp-picas)
 *
...
/* PiCAS priority assignment: timer=10, subscriptions=20+i */
#define PICAS_TIMER_PRIORITY     10
#define PICAS_SUB_BASE_PRIORITY  20
...
struct TimerNode : public rclcpp::Node
{
  ...
  void on_timer()
  {
    ...
    if (CHAIN_LENGTH == 1) {
      ...
      printf("[E3IP-CSV] %d,%u,%lld,0,0,0,0,0,0,0\n",
         ...
         rclcpp::NodeOptions().use_intra_process_comms(false))
  {
    ...
    sub_ = this->create_subscription<std_msgs::msg::Int32>(
      ...
      [](std_msgs::msg::Int32::UniquePtr msg) {
        ...
        printf("[E3IP-CSV] %d,%u,%lld,%lld,0,0,0,0,%lld,0\n",
               CHAIN_LENGTH, g_round,
               (long long)e2e, (long long)pub, (long long)sched);

        g_round++;
        if (g_round >= TOTAL_ROUNDS && !g_done.exchange(true)) {
          printf("[E3IP-SUMMARY] chain_len=%d "
...
int main(int argc, char * argv[])
{
  ...
  printf("=== Exp3inter-Picas: Task Chain (n=%d) SingleThreadedExecutor+PiCAS ===\n", CHAIN_LENGTH);
  printf("Period: %dms, Rounds: %u\n", TIMER_PERIOD_MS, TOTAL_ROUNDS);
  printf("[E3IP-CSV] chain_len,round,e2e_ns,publish_ns,gc_trigger_ns,invoke_cb_ns,getnext_ns,take_data_ns,sched_ns,dispatch_ns\n");

  rclcpp::init(argc, argv);

  rclcpp::executors::SingleThreadedExecutor executor;

  // Enable PiCAS priority-based callback scheduling
  executor.enable_callback_priority();

  // Set executor RT priority (SCHED_FIFO)
  executor.set_executor_priority_cpu(90, -1);

  auto timer_node = std::make_shared<TimerNode>();
  executor.add_node(timer_node);

  // Set timer callback priority
  executor.set_callback_priority(timer_node->timer_, PICAS_TIMER_PRIORITY);
  ...
  for (int i = 1; i < CHAIN_LENGTH - 1; i++) {
    ...
    // Set subscription callback priority (decreasing along chain)
    executor.set_callback_priority(node->sub_, PICAS_SUB_BASE_PRIORITY + i);
  }
  ...
  if (CHAIN_LENGTH >= 2) {
    ...
    // Set final subscription callback priority
    executor.set_callback_priority(final_node->sub_, PICAS_SUB_BASE_PRIORITY + CHAIN_LENGTH - 1);
  }
