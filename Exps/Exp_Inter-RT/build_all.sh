#!/bin/bash
#
# build_all.sh — Compile and link all 5 chain lengths for Exp_Inter-RT
# Uses rclcpp-rtss25 librclcpp.a (with RTExecutor)
#
set -euo pipefail
...
# rtss25-built librclcpp.a (contains RTExecutor)
...
for N in 2 3 4 5 10 15; do
    MAIN_OBJ="$BUILD_DIR/src/exp3inter_rt_main.cpp.${IDX}.o"
    if [ ! -f "$MAIN_OBJ" ]; then
...
# Step 2: Link all 5 executables using rtss25 librclcpp.a
LIBS=(
    -Wl,--start-group
...
for N in 2 3 4 5 10 15; do
    ...
    OUTPUT="$BUILD_DIR/exp3inter_rt_n${N}.exe"
