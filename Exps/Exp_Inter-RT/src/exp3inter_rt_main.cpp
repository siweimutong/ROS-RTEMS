/**
 * @file exp3inter_rt_main.cpp
 *
 * Exp3inter-RT: Task Chain Latency — RTExecutor (rclcpp-rtss25)
 *
 * Same chain topology as Exp3-ST, but uses RTExecutor which dispatches
...
struct TimerNode : public rclcpp::Node
{
  ...
  void on_timer(int channel)
  {
    ...
    if (CHAIN_LENGTH == 1) {
      ...
      printf("[E3IRT-CSV] %d,%u,%lld,0,0,0,0,0,0,0\n",
         ...
         rclcpp::NodeOptions().use_intra_process_comms(true))
  {
    ...
    sub_ = this->create_subscription<std_msgs::msg::Int32>(
      ...
      [](std_msgs::msg::Int32::UniquePtr msg) {
        ...
        printf("[E3IRT-CSV] %d,%u,%lld,%lld,0,0,0,0,%lld,0\n",
               CHAIN_LENGTH, g_round,
               (long long)e2e, (long long)pub, (long long)sched);

        g_round++;
        if (g_round >= TOTAL_ROUNDS && !g_done.exchange(true)) {
          printf("[E3IRT-SUMMARY] chain_len=%d "
...
int main(int argc, char * argv[])
{
  ...
  printf("=== Exp3inter-RT: Task Chain (n=%d) RTExecutor ===\n", CHAIN_LENGTH);
  printf("Period: %dms, Rounds: %u\n", TIMER_PERIOD_MS, TOTAL_ROUNDS);
  printf("[E3IRT-CSV] chain_len,round,e2e_ns,publish_ns,gc_trigger_ns,invoke_cb_ns,getnext_ns,take_data_ns,sched_ns,dispatch_ns\n");
