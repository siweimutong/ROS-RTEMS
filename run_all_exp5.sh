#!/bin/bash
#
# run_all_exp5.sh — Run sequentially Exp5-ST, Exp5-MT, Exp5-Picas, Exp5-ROSRT, Exp5-Ours
#
# Run from host; launches QEMU inside the rtems6-dev container via docker exec
# Each experiment calls ./run.sh all (s=1..5); QEMU and CSV output saved in each Result/ dir
#
set -euo pipefail
...
for exp in "${EXPERIMENTS[@]}"; do
    cleanup_qemu
    echo -e "${CYAN}┌──────────────────────────────────────────────────────────┐${NC}"
    echo -e "${CYAN}│  Running: ${exp}  (estimated ~10-20 minutes, 5 subscriber counts)${NC}"
    echo -e "${CYAN}└──────────────────────────────────────────────────────────┘${NC}"
    ...
    # Run inside container (QEMU x5 subscriber counts, max 600s per count)
...
echo "============================================================"
echo -e "  ${YELLOW}Exp5 Run Summary${NC}"
