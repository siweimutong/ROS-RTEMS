...
source_file="$HOME/RTEMS/quick-start/app/workspace/unique_identifier_msgs/config/config_unique_identifier_msgs__rosidl_typesupport_introspection_cpp/tmp/build/arm-rtems6-realview_pbx_a9_qemu/libunique_identifier_msgs__rosidl_typesupport_introspection_cpp.a"  # Source file (with full path)
...
if [ -f "${target_file}" ]; then
    rm -f "${target_file}"  # -f Force delete, avoids an error when the file is not found
