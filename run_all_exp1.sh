#!/bin/bash
#
# run_all_exp1.sh — Run sequentially Exp1-ST, Exp1-Picas, Exp1-ROSRT, Exp1-Ours
#
# Run from host; launches QEMU inside the rtems6-dev container via docker exec
# Each experiment calls ./run.sh run; QEMU and CSV output saved in each Result/ dir
#
set -euo pipefail
...
for exp in "${EXPERIMENTS[@]}"; do
    cleanup_qemu
    echo -e "${CYAN}┌──────────────────────────────────────────────────────────┐${NC}"
    echo -e "${CYAN}│  Running: ${exp}  (estimated ~5-10 minutes)${NC}"
    echo -e "${CYAN}└──────────────────────────────────────────────────────────┘${NC}"
    ...
    # Run inside container (single exe, max 600s)
    # timeout --foreground allows SIGINT to propagate to docker exec
    if timeout --foreground 600 docker exec -i "$CONTAINER" bash -lc \
        "cd ${WORKSPACE}/${exp} && ./run.sh run"; then
...
echo "============================================================"
echo -e "  ${YELLOW}Exp1 Run Summary${NC}"
