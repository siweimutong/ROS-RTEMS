...
def build(bld):  
    ...
    bld.stlib(
        target="tinyxml2",                 # Output library name  
        source=["init.c", "../../code/src/tinyxml2.cpp"],  # Source file  
        includes=["../../code/include"], #Header files
        cxxflags="-g -O2 -fPIC -fpermissive -fexceptions",                # C++ compile flags  
        lib=["stdc++"]                   # Link the standard C++ library  
    )  
    ' > wscript
    cd ..
fi
