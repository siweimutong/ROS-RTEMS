...
public:
     ...
     * impose further limits on the size of the returned 鈥榣ist.鈥�
     *
     ...
     * NOT YET IMPLEMENTED
     *
     * This operation accesses via 鈥榬ead鈥� the samples that match the criteria specified in the ReadCondition.
     ...
     *
     * In case the ReadCondition is a 鈥榩lain鈥� ReadCondition and not the specialized QueryCondition, the
     ...
     * 鈥榮mallest鈥� @c instance_handle among the ones that verify (a) @c instance_handle >= @c previous_handle and (b) have samples
     ...
     * to read, the @ref read_next_instance_w_condition operation may 鈥榣oan鈥� elements to the output collections which
     ...
     *
     * The read_next_sample operation provides a simplified API to 鈥榬ead鈥� samples avoiding the need for the
     ...
     * NOT YET IMPLEMENTED
     *
     * This operation is analogous to @ref read_w_condition except it accesses samples via the 鈥榯ake鈥� operation.
     *
     * The specified ReadCondition must be attached to the DataReader; otherwise the operation will fail and return
     * RETCODE_PRECONDITION_NOT_MET.
     *
     ...
     * This operation accesses a collection of Data values from the DataReader. The behavior is identical to
     * @ref read_next_instance except that all samples returned satisfy the specified condition. In other words, on
     * success all returned samples belong to the same instance, and the instance is the instance with 鈥榮mallest鈥�
    ...
    /**
     * @brief This operation copies the next, non-previously accessed Data value from the DataReader and 鈥榬emoves鈥� it
     * from the DataReader so it is no longer accessible. The operation also copies the corresponding SampleInfo.
     *
     * This operation is analogous to @ref read_next_sample except for the fact that the sample is 鈥榬emoved鈥� from the
     * DataReader.
     *
     ...
     *
     * This operation provides a simplified API to 鈥檛ake鈥� samples avoiding the need for the application to manage
     ...
     * DataReader.
     *
     * The @c data_values and @c sample_infos must belong to a single related 鈥榩air鈥�; that is, they should correspond
    ...
    /**
     * This operation deletes all the entities that were created by means of the 鈥渃reate鈥� operations on the DataReader.
