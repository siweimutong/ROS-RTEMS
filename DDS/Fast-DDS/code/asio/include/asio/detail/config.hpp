...
/*
 * SylixOS epoll 接口运行存在问题，不定义 ASIO_HAS_EPOLL 后 DDS 会使用 select 接口替代
 */
#ifndef SYLIXOS
...
#  if defined(ASIO_HAS_EPOLL)
#   if (defined(SYLIXOS)) || (__GLIBC__ > 2) || (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 8)
...
#  elif defined(__GNUC__) && !defined(__MINGW32__) \
     && !defined(linux) && !defined(__linux) && !defined(__linux__) && !defined(SYLIXOS)
#   define ASIO_HAS_THREADS 1
