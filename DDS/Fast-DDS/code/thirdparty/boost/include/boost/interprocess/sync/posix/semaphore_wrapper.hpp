...
//#ifdef 0
    /*
     * 此处信号量可能会通过共享内存在多个进程中共享使用，因此需要使用全局对象
     */
#define __PX_UNNAME_SEM_NAME        "?psem_unname"
#define __PX_UNNAME_SEM_OPTION      (LW_OPTION_WAIT_PRIORITY | LW_OPTION_SIGNAL_INTER)
#define LW_OPTION_OBJECT_GLOBAL      0x80000000

//typedef SEM_ulSemaphore SEM_pvPxSem
//    int ret = 0;
//
//    handle->SEM_ulPad[0] = Lw_SemaphoreC_Create(__PX_UNNAME_SEM_NAME, initialCount,
//                                                   __ARCH_INT_MAX,
//                                                   __PX_UNNAME_SEM_OPTION | LW_OPTION_OBJECT_GLOBAL,
//                                                   LW_NULL);            /*  创建信号量                  */
//    if (handle->SEM_ulPad[0] == LW_OBJECT_HANDLE_INVALID) {
//        errno = ENOSPC;
//        ret = -1;
//    } else {
//        handle->SEM_pvPxSem = LW_NULL;
////        handle->SEM_ulPad[0] = LW_FALSE;
//        handle->SEM_presraw = LW_FALSE;
//
//    }
