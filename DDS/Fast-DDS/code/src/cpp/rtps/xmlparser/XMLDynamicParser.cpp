...
#ifdef SYLIXOS
            || disc.compare(DDS_INT8) == 0
            || disc.compare(DDS_UINT8) == 0)
#else
            || disc.compare(INT8) == 0
...
#ifdef SYLIXOS
    else if (disc.compare(DDS_LONG) == 0)
#else
...
#ifdef SYLIXOS
    else if (disc.compare(DDS_ULONG) == 0)
#else
...
#ifdef SYLIXOS
    else if (disc.compare(DDS_CHAR) == 0)
#else
...
#ifdef SYLIXOS
    else if (strncmp(memberType, DDS_CHAR, 5) == 0)
#else
...
#ifdef SYLIXOS
            || strncmp(memberType, DDS_UINT8, 6) == 0
            || strncmp(memberType, DDS_INT8, 5) == 0)
#else
...
#ifdef SYLIXOS
    else if (strncmp(memberType, DDS_LONG, 5) == 0)
    {
        memberBuilder = factory->create_int32_builder();
    }
    else if (strncmp(memberType, DDS_ULONG, 13) == 0)
#else
