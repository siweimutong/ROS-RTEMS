...
if [[ "$flag" == 1 ]]; then
    ...
    */
    ...
    # python ../mk_to_wscript_static.py ../libfastcdr.mk wscript
...
def build(bld):
    ...
    # fastcdr target (static library)
    # Compile and link static library
    bld.stlib(
        target="fastcdr",
        source=[
            "init.c",
            "../../code/src/cpp/Cdr.cpp",
            "../../code/src/cpp/FastCdr.cpp",
            "../../code/src/cpp/FastBuffer.cpp",
            "../../code/src/cpp/exceptions/Exception.cpp",
            "../../code/src/cpp/exceptions/NotEnoughMemoryException.cpp",
            "../../code/src/cpp/exceptions/BadParamException.cpp"
        ],
        includes=[
            "../../code/include",
            "../../code/build/include"
        ],
        cxxflags="-fpermissive -fexceptions -frtti -std=c++17 -Wno-register -g -Wno-unknown-pragmas -fPIC",
        linkflags="-static",
        lib=["foonathan_memory", "stdc++", "rtemscpu", "rtemsbsp", "rtemsdefaultconfig", "m", "gcc", "c"],
    ) 
    ' > wscript
fi
