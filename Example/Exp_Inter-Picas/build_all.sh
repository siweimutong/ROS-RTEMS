#!/bin/bash
#
# build_all.sh — Compile once, then link all 5 chain lengths
# Uses rclcpp-picas (with PiCAS priority scheduling) instead of standard rclcpp
#
set -euo pipefail
...
# PiCAS-built librclcpp.a path (instead of standard BSP lib)
PICAS_LIB_DIR="../../rclcpp-picas/config/tmp/build/arm-rtems6-realview_pbx_a9_qemu"
...
for N in 2 3 4 5 10 15; do
    ...
    MAIN_OBJ="$BUILD_DIR/src/exp3inter_picas_main.cpp.${IDX}.o"
...
# Step 2: Link all 5 executables using PiCAS librclcpp.a
LIBS=(
    -Wl,--start-group
...
echo "[2/2] Linking all executables (using PiCAS librclcpp.a)..."
IDX=1
for N in 2 3 4 5 10 15; do
    TEST_OBJ="$BUILD_DIR/src/test_main.cpp.${IDX}.o"
    MAIN_OBJ="$BUILD_DIR/src/exp3inter_picas_main.cpp.${IDX}.o"
    OUTPUT="$BUILD_DIR/exp3inter_picas_n${N}.exe"
