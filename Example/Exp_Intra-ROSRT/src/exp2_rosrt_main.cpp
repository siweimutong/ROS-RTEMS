/**
 * @file exp2_rosrt_main.cpp
 *
 * Exp2-ROSRT: Task Chain Latency — NoExecutor (rclcpp-rtss25)
 *
 * Same chain topology as Exp2-ST/MT, but uses NoExecutor which
...
struct TimerNode : public rclcpp::Node
{
  ...
  void on_timer()
  {
    ...
    if (CHAIN_LENGTH == 1) {
      ...
      printf("[E2ROSRT-CSV] %d,%u,%lld,0,0,0,0,0,0,0\n",
         ...
         rclcpp::NodeOptions().use_intra_process_comms(true))
  {
    ...
    sub_ = this->create_subscription<std_msgs::msg::Int32>(
      ...
      [](std_msgs::msg::Int32::UniquePtr msg) {
        ...
        printf("[E2ROSRT-CSV] %d,%u,%lld,%lld,0,0,0,0,%lld,0\n",
               CHAIN_LENGTH, g_round,
               (long long)e2e, (long long)pub, (long long)sched);

        g_round++;
        if (g_round >= TOTAL_ROUNDS + WARMUP_ROUNDS && !g_done.exchange(true)) {
          printf("[E2ROSRT-SUMMARY] chain_len=%d "
...
int main(int argc, char * argv[])
{
  ...
  printf("=== Exp2-ROSRT: Task Chain (n=%d) NoExecutor ===\n", CHAIN_LENGTH);
  printf("Period: %dms, Rounds: %u\n", TIMER_PERIOD_MS, TOTAL_ROUNDS);
  printf("[E2ROSRT-CSV] chain_len,round,e2e_ns,publish_ns,gc_trigger_ns,invoke_cb_ns,getnext_ns,take_data_ns,sched_ns,dispatch_ns\n");
