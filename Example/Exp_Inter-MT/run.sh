...
[ -f "$BUILD_DIR/exp3inter_mt_n2.exe" ] || ./build_all.sh

run_one() {
    local N="$1"
    local EXE="exp3inter_mt_n${N}.exe"
    ...
    local RAW="${RESULT_DIR}/exp3inter_mt_n${N}_raw.txt"
    ...
    while kill -0 $QPID 2>/dev/null; do
        ...
        if [ "$SUMMARY_SEEN" -eq 0 ] && grep -q '\[E3IMT-SUMMARY\]' "$RAW" 2>/dev/null; then
            SUMMARY_SEEN=1
    ...
    # Record peak memory
    echo "exp3inter_mt_n${N}  vmhwm_kb=${PEAK_HWM}  vmpeak_kb=${PEAK_VMSIZE}" >> "${RESULT_DIR}/memory_peaks.txt"

    # Extract CSV lines (header + data) into single csv file
    grep '^\[E3IMT-CSV\]' "$RAW" | sed 's/^\[E3IMT-CSV\] //' > "${RESULT_DIR}/exp3inter_mt_n${N}.csv" || {
        echo "WARNING: no CSV lines found for n=${N}, checking raw output..."
        wc -l "$RAW" 2>/dev/null || true
    }

    local LINES=$(wc -l < "${RESULT_DIR}/exp3inter_mt_n${N}.csv" 2>/dev/null || echo 0)
    echo "  n=${N}: ${LINES} lines  peak_rss=${PEAK_HWM}kB  peak_vm=${PEAK_VMSIZE}kB"
    if [ "$LINES" -gt 0 ]; then
        head -3 "${RESULT_DIR}/exp3inter_mt_n${N}.csv"
    fi
}
