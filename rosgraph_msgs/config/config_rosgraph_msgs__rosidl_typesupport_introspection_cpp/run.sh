...
source_file="$HOME/RTEMS/quick-start/app/workspace/rosgraph_msgs/config/config_rosgraph_msgs__rosidl_typesupport_introspection_cpp/tmp/build/arm-rtems6-realview_pbx_a9_qemu/librosgraph_msgs__rosidl_typesupport_introspection_cpp.a"  # Source file (with full path)
...
if [ -f "${target_file}" ]; then
    rm -f "${target_file}"  # -f forces deletion and avoids errors when the file is not found
