...
source_file="$HOME/RTEMS/quick-start/app/workspace/rosgraph_msgs/config/config_rosgraph_msgs__rosidl_typesupport_fastrtps_cpp/tmp/build/arm-rtems6-realview_pbx_a9_qemu/librosgraph_msgs__rosidl_typesupport_fastrtps_cpp.a"  # Source file (with full path)
...
if [ -f "${target_file}" ]; then
    rm -f "${target_file}"  # -f Force delete to avoid an error when the file is not found
