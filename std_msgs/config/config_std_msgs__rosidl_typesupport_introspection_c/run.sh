...
source_file="$HOME/RTEMS/quick-start/app/workspace/std_msgs/config/config_std_msgs__rosidl_typesupport_introspection_c/tmp/build/arm-rtems6-realview_pbx_a9_qemu/libstd_msgs__rosidl_typesupport_introspection_c.a"  # Source file (with full path)
...
if [ -f "${target_file}" ]; then
    rm -f "${target_file}"  # -f Force delete, avoids error when the file is not found
