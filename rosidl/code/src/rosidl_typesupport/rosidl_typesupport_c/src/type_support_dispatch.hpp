  ...
  const TypeSupport * handle, const char * identifier)
{
  ...
  if (handle->typesupport_identifier == rosidl_typesupport_c__typesupport_identifier) {
    ...
    for (size_t i = 0; i < map->size; ++i) {
      ...
      int flag = ObtainPointerC::getInstance()->isMsgOrSrv(map->symbol_name[i]);
      MsgTypeSupportFunc msg_pt = nullptr;
      SrvTypeSupportFunc srv_pt = nullptr;

      if (flag == 0){
        msg_pt = ObtainPointerC::getInstance()->getMsgFunc(map->symbol_name[i]);
        if (msg_pt) printf("C Msg not NULL\r\n");
        else printf("C Msg is NULL\r\n");
        ...
        srv_pt = ObtainPointerC::getInstance()->getSrvFunc(map->symbol_name[i]);
        if (srv_pt) printf("C Srv not NULL\r\n");
        else printf("C Srv is NULL\r\n");
