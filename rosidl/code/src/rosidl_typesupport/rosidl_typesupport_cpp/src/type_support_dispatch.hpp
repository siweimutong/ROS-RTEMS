...
{
// extern const rosidl_message_type_support_t * rosidl_typesupport_fastrtps_cpp__get_message_type_support_handle__rmw_dds_common__msg__ParticipantEntitiesInfo();
  ...
  const TypeSupport * handle, const char * identifier) noexcept
{
  ...
  if (handle->typesupport_identifier == rosidl_typesupport_cpp::typesupport_identifier) {
    ...
    for (size_t i = 0; i < map->size; ++i) {
      ...
      int flag = ObtainPointerCpp::getInstance()->isMsgOrSrv(map->symbol_name[i]);
      MsgTypeSupportFunc msg_pt = nullptr;
      SrvTypeSupportFunc srv_pt = nullptr;
      if (flag == 0){
        msg_pt = ObtainPointerCpp::getInstance()->getMsgFunc(map->symbol_name[i]);
        if (msg_pt) printf("CPP Msg not nullptr\r\n");
        else printf("CPP Msg is nullptr\r\n");

        typedef const TypeSupport * (* funcSignature)(void);
        funcSignature func = reinterpret_cast<funcSignature>(msg_pt);
        const TypeSupport * ts = func();
        // const TypeSupport * ts = reinterpret_cast<const TypeSupport*>(rosidl_typesupport_fastrtps_cpp__get_message_type_support_handle__rmw_dds_common__msg__ParticipantEntitiesInfo());
        return ts;
      }else{
        srv_pt = ObtainPointerCpp::getInstance()->getSrvFunc(map->symbol_name[i]);
        if (srv_pt) printf("CPP Srv not nullptr\r\n");
        else printf("CPP Srv is nullptr\r\n");

        typedef const TypeSupport * (* funcSignature)(void);
        funcSignature func = reinterpret_cast<funcSignature>(srv_pt);
        const TypeSupport * ts = func();
        // const TypeSupport * ts = reinterpret_cast<const TypeSupport*>(rosidl_typesupport_fastrtps_cpp__get_message_type_support_handle__rmw_dds_common__msg__ParticipantEntitiesInfo());
        return ts;
      }
