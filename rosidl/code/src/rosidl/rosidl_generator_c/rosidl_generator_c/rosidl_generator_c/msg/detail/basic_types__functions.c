  ...
  rosidl_generator_c__msg__BasicTypes__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rosidl_generator_c__msg__BasicTypes * data =
      (rosidl_generator_c__msg__BasicTypes *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rosidl_generator_c__msg__BasicTypes__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rosidl_generator_c__msg__BasicTypes__fini(&output->data[i]);
        }
        return false;
      }
    }
