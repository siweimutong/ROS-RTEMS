...
#include <rtems.h>
#include <cstdio>
/* Exp2.3 instrumentation: per-call raw measurement of get_next_ready_executable */
...
/* Exp2.3 instrumentation: measure wait_for_work overhead */
volatile uint64_t g_wait_for_work_ns = 0;
volatile int g_wait_for_work_calls = 0;
/* Exp2.3: per-call raw output control (set by test_main) */
volatile int g_exp23_raw_enabled = 0;
...
/* Exp2.3: duration of last wait_for_work */
...
Executor::wait_for_work(std::chrono::nanoseconds timeout)
{
  ...
  clock_gettime(CLOCK_MONOTONIC, &_wfw_t1);
  {
    ...
    uint64_t dur = (uint64_t)(_total_ns - _rcl_wait_ns);
    g_wait_for_work_ns += dur;
    g_wait_for_work_calls++;
    g_wait_dur_ns = dur;
    if (g_exp23_raw_enabled) {
      double dur_us = (double)dur / 1000.0;
      printf("[E2.3-WAIT] %d,%.3f\n", g_wait_for_work_calls, dur_us);
    }
  }
#endif
}
