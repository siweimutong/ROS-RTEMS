...
#else
static int sched_setattr_rtems(pid_t pid, const struct sched_attr *attr)
{
  if (attr->sched_policy == SCHED_FIFO || attr->sched_policy == SCHED_RR) {
    struct sched_param param;
    param.sched_priority = attr->sched_priority;
    return pthread_setschedparam((pthread_t)pid, attr->sched_policy, &param);
  }
  return 0;
}
#endif
...
#else
    int ret = sched_setattr_rtems((pid_t)pthread_self(), &rt_attr);
#endif
    if (ret < 0) {
