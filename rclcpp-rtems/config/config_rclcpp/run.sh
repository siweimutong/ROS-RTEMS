...
source_file="$HOME/RTEMS/quick-start/app/workspace/rclcpp-rtss25/config/config_rclcpp/tmp/build/arm-rtems6-realview_pbx_a9_qemu/librclcpp.a"
...
if [ -f "${target_file}" ]; then
    rm -f "${target_file}"
    echo "Removed existing target file: ${target_file}" >> $log_file_path 2>&1
fi
