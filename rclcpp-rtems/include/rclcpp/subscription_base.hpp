...
public:
  ...
  /// Get the currently registered DDS on-new-message callback (rclcpp-rtems).
  /**
   * Returns a copy of the stored callback so that RTExecutor can wrap it
   * with additional notification logic (e.g., rtems_event_send for DDS path).
   *
   * \return the current on_new_message_callback, or an empty function if none
   */
  std::function<void(size_t)>
  get_on_new_message_callback()
  {
    std::lock_guard<std::recursive_mutex> lock(callback_mutex_);
    return on_new_message_callback_;
  }

  /// Set a direct notification callback for intra-process messages (bypasses rcl_wait).
  /**
   * When set, the intra-process publishing path will call this callback
   * directly instead of triggering the guard condition.  This enables
   * OS-kernel-driven notification (e.g., rtems_event_send to a CallbackTask)
   * without any user-level detection loop.
   *
   * Pass nullptr to revert to standard guard condition mode.
   */
  void
  set_intra_process_direct_notify(std::function<void()> callback)
  {
    if (!use_intra_process_ || !subscription_intra_process_) {
      return;
    }
    subscription_intra_process_->set_direct_notify_callback(std::move(callback));
  }
