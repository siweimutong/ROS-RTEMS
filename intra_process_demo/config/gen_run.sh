...
if [ -d "${target_dir}" ]; then
    ...
    echo "No tmp directory in the current directory, nothing to delete"
fi
...
def build(bld):
    ...
    # intra_process_demo.exe target (executable)
    bld.program(
        target="intra_process_demo.exe",
        source=[
            "../test_main.cpp",
            "../../code/src/two_node_pipeline/two_node_pipeline.cpp",
        ],
        includes=[
            "../../code/include",
