...
//  }
  ...
  printf("Running experiment from now on for %lld %s\n", EXPERIMENT_DURATION.count(), " seconds ...");
  ...
  printf("High priority executor thread ran for %lld ms.\n", high_prio_thread_duration_ms);
  printf("Low priority executor thread ran for %lld ms.\n", low_prio_thread_duration_ms);
