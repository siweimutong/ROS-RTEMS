...
log_file_path="$log_path/compile_log_rmw_implementation.txt"
...
# python ../mk_to_wscript_static.py ../librmw_implementation.mk wscript
...
def build(bld):
    ...
    # rmw_implementation target (static library)
    # Compile and link static library
    bld.stlib(
        target="rmw_implementation",
        source=[
            "init.c",
            "../../code/src/rmw_implementation/rmw_implementation/src/functions.cpp"
        ],
        includes=[
        ...
        linkflags="-static",
        lib=["rcpputils", "rcutils", "rmw", "ament_index_cpp", "stdc++", "rtemscpu", "rtemsbsp", "rtemsdefaultconfig", "m", "gcc", "c"],
    )
' > wscript
fi
exit 0