...
def generate_rtems_wscript(config, output_file):
    ...
    if config['target']:
        ...
        if is_library:
            ...
            # Add dependency libraries (remove .so suffix)
...
def convert_mk_to_wscript(mk_file, output_file=None):
    ...
    if output_file is None:
        output_file = os.path.join(os.path.dirname(mk_file), "wscript")
...
def replace_src_with_source(file_path, strs, target):
    """
    ...
    for line in lines:
        ...
        if 'source=[' in line:
            ...
            new_line = line.replace(strs, target)
            if new_line != line:
                modified = True
    ...
    if modified:
        ...
        # print(f"Done! Replaced 'src' with 'source' in source/includes lines of file {file_path}")
    else:
        print("No 'src' content to replace found, file unchanged")
def replace_src_with_include(file_path, strs, target):
    """
    Replace 'src' with 'source' in source=[] and includes=[] lines
...
if __name__ == "__main__":
    ...
    replace_src_with_source(path, 'src/Fast-DDS', '../../code')
    replace_src_with_include(path, 'src/Fast-DDS', '../../code')
    replace_src_with_include(path, 'src/Fast-CDR', '../../../Fast-CDR/code')
    replace_src_with_include(path, 'src/memory-main', '../../../memory-main/code')
    replace_src_with_include(path, 'src/asio', '../../code/asio')
    replace_src_with_include(path, 'src/tinyxml2', '../../../tinyxml2/code/include')