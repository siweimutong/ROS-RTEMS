...
log_file_path="$log_path/compile_log_builtin_interfaces.txt"

if [[ ! -d "$log_path" ]]; then
    mkdir -p "$log_path"  # -pis key: creates build even if no subdirectories exist under root_path
fi

# Check if the file is not found; create it if so
...
if [[ "$flag" == 1 ]]; then
    ...
    if eval $GIT_CMD; then
        ...
        echo "❌ Failed because: network problem / repository not found / path already exists / insufficient permissions" >> $log_file_path 2>&1
...
# python ../mk_to_wscript_static.py ../libbuiltin_interfaces__rosidl_typesupport_fastrtps_c.mk wscript
...
def bsp_configure(conf, arch_bsp):
    # You can perform BSP-related configuration checks here
    pass
...
def build(bld):
    ...
    # builtin_interfaces__rosidl_typesupport_fastrtps_c target (static library)
    # Compile and link static library
    bld.stlib(
        target="builtin_interfaces__rosidl_typesupport_fastrtps_c",
        source=[
            "init.c",
            "../../../code/src/builtin_interfaces/rosidl_typesupport_fastrtps_c/builtin_interfaces/msg/detail/duration__type_support_c.cpp",
            "../../../code/src/builtin_interfaces/rosidl_typesupport_fastrtps_c/builtin_interfaces/msg/detail/time__type_support_c.cpp"
        ],
        includes=[
            "../../../code/src/builtin_interfaces/rosidl_typesupport_fastrtps_c",
            ...
            "../../../code/src/builtin_interfaces/rosidl_generator_c"
        ],
        defines=[
        ...
        linkflags="-static",
        lib=["rosidl_runtime_c", "rosidl_typesupport_fastrtps_c", "rosidl_typesupport_fastrtps_cpp", "fastcdr", "builtin_interfaces__rosidl_generator_c", "stdc++", "rtemscpu", "rtemsbsp", "rtemsdefaultconfig", "m", "gcc", "c"],
    )
' > wscript
fi
exit 0