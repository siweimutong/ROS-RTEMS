...
source_file="$HOME/RTEMS/quick-start/app/workspace/builtin_interfaces/config/config_builtin_interfaces__rosidl_typesupport_introspection_cpp/tmp/build/arm-rtems6-realview_pbx_a9_qemu/libbuiltin_interfaces__rosidl_typesupport_introspection_cpp.a"  # Source file (with full path)
...
if [ -f "${target_file}" ]; then
    rm -f "${target_file}"  # -f Force delete to avoid errors when the file is not found
