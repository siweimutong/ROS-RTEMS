...
source_file="$HOME/RTEMS/quick-start/app/workspace/rcl_interfaces/config/config_rcl_interfaces__rosidl_generator_c/tmp/build/arm-rtems6-realview_pbx_a9_qemu/librcl_interfaces__rosidl_generator_c.a"  # Source file (with full path)
...
if [ -f "${target_file}" ]; then
    rm -f "${target_file}"  # -f Force delete, avoid error if the file is not found
