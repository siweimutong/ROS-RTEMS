...
public:
  ...
  /// Set a direct notification callback (bypasses guard condition / rcl_wait).
  /**
   * When set, trigger_guard_condition() will NOT trigger the guard condition,
   * and invoke_on_new_message() will call this callback instead of
   * on_new_message_callback_.  This enables OS-kernel-driven notification
   * (e.g., rtems_event_send to a CallbackTask) without any user-level
   * detection loop (rcl_wait / spin).
   *
   * Pass nullptr to revert to standard guard condition mode.
   */
  void
  set_direct_notify_callback(std::function<void()> callback)
  {
    std::lock_guard<std::recursive_mutex> lock(callback_mutex_);
    direct_notify_callback_ = std::move(callback);
  }

protected:
  std::recursive_mutex callback_mutex_;
  std::function<void(size_t)> on_new_message_callback_ {nullptr};
  std::function<void()> direct_notify_callback_ {nullptr};
  ...
  invoke_on_new_message()
  {
    ...
    if (this->direct_notify_callback_) {
      /* Direct notification mode: bypass guard condition entirely.
       * The callback (e.g., rtems_event_send) directly wakes the
       * consumer task — no rcl_wait, no spin loop needed. */
      this->direct_notify_callback_();
    } else if (this->on_new_message_callback_) {
