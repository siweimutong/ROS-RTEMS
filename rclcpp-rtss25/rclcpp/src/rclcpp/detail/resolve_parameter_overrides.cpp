...
#ifdef RTEMS
/* rcl 5.3.7 does not export rcl_arguments_get_param_overrides.
 * Provide a weak stub so the linker succeeds; it returns NOT_FOUND
 * and resolve_parameter_overrides will skip it gracefully. */
#include "rcl/arguments.h"
extern "C" __attribute__((weak))
rcl_ret_t rcl_arguments_get_param_overrides(const rcl_arguments_t * args, rcl_params_t ** params)
{
  (void)args;
...
#ifdef RTEMS
    /* rcl 5.3.7 does not provide rcl_arguments_get_param_overrides.
     * Stub: skip parameter overrides on RTEMS. */
    if (RCL_RET_OK != ret) {
      continue;
    }
#else
