...
volatile int64_t  g_posix_timer_base_ns = 0;  /* clock_gettime right before first timer_settime() */
...
NoExecutor::start() {
  ...
  started = true;
  /* Record baseline time right before arming the first POSIX timer.
   * User callbacks can read g_posix_timer_base_ns as the definitive
   * "time zero" — POSIX timer expires at:
   *   g_posix_timer_base_ns + it_value + n * it_interval + latency_n
   * Since latency_n >= 0, observed deviation from theory is always >= 0. */
  {
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
    g_posix_timer_base_ns = (int64_t)ts.tv_sec * 1000000000LL + (int64_t)ts.tv_nsec;
  }
  for (PosixTimer *timer: timers) {
